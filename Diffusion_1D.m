...
UnL=-3.2;               %Left Neumann B.C (du/dn=UnL) 
UnR=-3.2;               %Right Neumann B.C (du/dn=UnR) 
...
for i=1:nx
    ...
    %}
    u(i)=Pmax*(1/(sigma*sqrt(2*pi)))*exp(-(x(i)-1)^2/(2*sigma^2));
end
...
i=2:nx-1;
for it=0:nt
    UnL=0.2*(abs(2*it-30)-abs(it-30));
    UnR=UnL;
    bc(1)=-UnL*vis*dt/dx; bc(nx-2)=UnR*vis*dt/dx;  %Neumann B.Cs
    un=u;
    h=plot(x,u);       %plotting the velocity profile
    axis([0 2 0 5])
    title({['1-D Diffusion with '],['time(\itt) = ',num2str(dt*it)]})
    xlabel('Spatial co-ordinate (x) \rightarrow')
    ylabel('fish population density (\rho) \rightarrow')
    drawnow; 
    refreshdata(h)
